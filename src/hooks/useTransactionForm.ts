...
const initialFormData: TransactionFormData = {
  tipo: "saida", subtipo: "despesa", descricao: "", valor: "",
  dataCompetencia: new Date(), dataVencimento: undefined, dataPagamento: undefined,
  status: "pendente", categoriaId: "", contaId: "", contaDestinoId: "",
  socioId: "", centroCustoId: "", observacoes: "",
  parcelado: false, numeroParcelas: 2, recorrente: false,
  frequenciaRecorrencia: "mensal", dataFimRecorrencia: undefined,
};

export interface ReservaPreview {
  id: string; nome: string; percentual: number; valorProvisionado: number; cor: string;
}
...
export function useTransactionForm() {
  ...
  function parseValor(): number {
    return parseFloat(formData.valor.replace(/[^\d,.-]/g, "").replace(",", ".")) || 0;
  }
    ...
    try {
      ...
      if (formData.parcelado && !formData.recorrente) {
        ...
        const parcelas = Array.from({ length: formData.numeroParcelas }, (_, i) => {
          const venc = formData.dataVencimento ? addMonths(formData.dataVencimento, i) : addMonths(formData.dataCompetencia, i);
          return {
            descricao: `${formData.descricao} (${i + 1}/${formData.numeroParcelas})`, valor: Math.round((valor / formData.numeroParcelas) * 100) / 100,
            data: format(formData.dataCompetencia, "yyyy-MM-dd"), tipo_movimentacao: tipoDb, subtipo: formData.subtipo,
            status: i === 0 ? formData.status : "pendente", categoria_id: formData.categoriaId || null, conta_id: formData.contaId || null,
      ...
      if (formData.recorrente) {
        ...
        const freqMonths = formData.frequenciaRecorrencia === "semanal" ? 0.25 : formData.frequenciaRecorrencia === "quinzenal" ? 0.5
          : formData.frequenciaRecorrencia === "mensal" ? 1 : formData.frequenciaRecorrencia === "bimestral" ? 2
          : formData.frequenciaRecorrencia === "trimestral" ? 3 : formData.frequenciaRecorrencia === "semestral" ? 6 : 12;
        const instances = [];
        for (let i = 0; i < 12; i++) {
          ...
          instances.push({
            descricao: formData.descricao, valor, data: format(dt, "yyyy-MM-dd"), tipo_movimentacao: tipoDb, subtipo: formData.subtipo,
      ...
      const { data: lancamento, error: lancErr } = await supabase.from("lancamentos").insert({
        descricao: formData.descricao, valor, data: format(formData.dataCompetencia, "yyyy-MM-dd"), tipo_movimentacao: tipoDb,
  ...
  return { formData, updateField, subtipos, showSocio, showContaDestino, saving, handleSubmit, reservaPreviews, showReservaDialog, setShowReservaDialog, confirmReservas, skipReservas };
}
